...
public class ProductsController : BaseApiController
{
    ...
    )
    {
        var query = _context
            ...
            .Filter(productParams.Meats, productParams.SideDishes)
            .AsQueryable();
    ...
    {
        var meats = await _context.Products.Select(p => p.Meat).Distinct().ToListAsync();
        var sideDishes = await _context.Products.Select(p => p.SideDish).Distinct().ToListAsync();

        return Ok(new { meats, sideDishes });
    }
